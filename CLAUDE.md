...
│   └── 変更調剤について.html 等
├── yakureki/index.html ← 薬歴マニュアル（2026-07-20公開。薬歴カテゴリの入口。基本記載ルール・個別指導対策・加算別記載一覧・ハイリスク7薬効群・文例集の17ページ。各ページは独自ヘッダー＋back-link運用で共通ヘッダー未搭載）
├── 薬歴の書き方_実例解説集.html ← 薬歴カテゴリの1ページ。編集ツール（draft/薬歴記載例_見直し.html）の保管庫JSONを取り込んで実例を追加していく
...
- **新規ページは必ず site-header.js の PAGES に登録する**（category＋title）。未登録だとサイト内検索・パンくずに出ない（2026-07-05の構成点検で10ページの漏れが見つかった）。カテゴリは `kaitei / kasan / checklist / tools / knowledge / yakureki / jimu`。
