...
```
index.html          ← ホーム（ヒーロー・コンテンツ一覧・おすすめ・最近の更新5件）
├── updates.html    ← 全更新履歴（2段組フォーマット）
├── 加算まとめ.html  ← 加算・管理料の恒久ハブ（検索窓・正式区分の4カテゴリ。育てる主軸）
│   └── 各加算・管理料の個別ページ（所属カテゴリ kasan。改定カードからもリンクされる）
├── checklists.html ← 算定チェックリスト集（印刷用。ナビ「チェックリスト」あり）
├── kaitei2026.html ← 2026改定 資料一覧（時限アーカイブ。疑義解釈・別表・改定固有の資料）
├── tools.html      ← 実務ツール集
│   └── gigi-search.html、届出判定ツール、別表１（１）など
├── knowledge.html  ← 実務ナレッジ集（改定非依存の実務知識・Q&A）
│   └── 変更調剤について.html 等
└── 事務スタッフ向け.html ← 読者別の棚（ナビには出さない。トップカード・検索・パンくずで到達）
```

- 戻る導線は共通ヘッダー（site-header.js）が自動で担う。手動の戻るボタンは置かない。
- index.html のコンテンツカード件数（「N資料収録」）は data-count-from による自動カウント。手動メンテ不要。
- **新規ページは必ず site-header.js の PAGES に登録する**（category＋title）。未登録だとサイト内検索・パンくずに出ない（2026-07-05の構成点検で10ページの漏れが見つかった）。カテゴリは `kaitei / kasan / checklist / tools / knowledge / jimu`。
- トップのカード順は「加算まとめ → チェックリスト → 2026改定 → ツール → ナレッジ → 事務スタッフ向け」（恒久コンテンツ優先。2026-07-05変更）。
...
**公開方針（2026-07-05決定）：** 別サイトに分けず、**同一サイトの第6カテゴリ「薬歴」として公開する**（ナビ追加＋トップに7枚目のカード）。理由：①サイト全体が noindex の社内専用なので分離する必要がない ②加算まとめの薬歴ブロックは「核＋薬歴マニュアルへのリンク」構造で、切り離すと相互リンク（算定の正本＝加算まとめ／薬歴の正本＝薬歴マニュアル）が壊れる。**公開前にまとめて設計するタスク（今はやらない）：** `yakureki/index.html` とルート `index.html` のファイル名衝突（site-header.js の PAGES はファイル名キー）／サブディレクトリの相対パス対応（ロゴ・カテゴリURL）／薬歴ページ群を共通ヘッダーに載せるか／検索対象に含めるか／draft昇格のタイミング。改定ページ（kaitei2026）からは導線を作らない。
