...
│   └── 変更調剤について.html 等
├── 薬歴の書き方_実例解説集.html ← 薬歴カテゴリ（2026-07-13先行新設）の入口を当面兼ねる。編集ツール（draft/薬歴記載例_見直し.html）の保管庫JSONを取り込んで実例を追加していく
...
- トップのカード順は「加算まとめ → チェックリスト → 2026改定 → ツール → ナレッジ → 薬歴 → 事務スタッフ向け」（恒久コンテンツ優先。薬歴カードは2026-07-13追加）。
...
**公開方針（2026-07-05決定）：** 別サイトに分けず、**同一サイトの第6カテゴリ「薬歴」として公開する**（ナビ追加＋トップに7枚目のカード）。**※薬歴カテゴリ（yakureki）自体は2026-07-13に「薬歴の書き方｜実例解説集」の公開に合わせて先行新設済み**（ナビ・トップカードあり。カテゴリ入口は当面このページ。マニュアル公開時に入口を yakureki のハブへ付け替える）。理由：①サイト全体が noindex の社内専用なので分離する必要がない ②加算まとめの薬歴ブロックは「核＋薬歴マニュアルへのリンク」構造で、切り離すと相互リンク（算定の正本＝加算まとめ／薬歴の正本＝薬歴マニュアル）が壊れる。**公開前にまとめて設計するタスク（今はやらない）：** `yakureki/index.html` とルート `index.html` のファイル名衝突（site-header.js の PAGES はファイル名キー）／サブディレクトリの相対パス対応（ロゴ・カテゴリURL）／薬歴ページ群を共通ヘッダーに載せるか／検索対象に含めるか／draft昇格のタイミング。改定ページ（kaitei2026）からは導線を作らない。
