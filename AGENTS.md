# 調剤報酬・薬歴まとめ サイト - Codex 作業ガイド

このファイルは別PCでの作業引き継ぎ用です。Codex がこのディレクトリを開いたとき自動的に読み込まれます。
...
> このファイルの構成図は 2026-07-20 のフォルダ整理より前の内容で古い。**現行の構成は `CLAUDE.md` を見る。**
> アイコン・ロゴ（favicon 各種・会社ロゴ.png）は 2026-08-02 から `assets/icons/`、薬価データは `assets/data-r86.js`、deploy.js は `tools-internal/` にある。
...
| アンバー `#b45309` 系 | `--amber` / `--amber-bg:#fdf0e3` / `--amber-bd:#e8c9a3` | **厳格化・制約・注意（＝赤の代わり）** |
| 赤 `#cc0000` | `--red`（定義は残してよい） | **使わない** |
| グレー文字 | `--txt-2:#444444` / `--txt-3:#5e6470` | 補助テキスト。**薄すぎ厳禁**（#999以下は「・」等の弱い記号のみ） |

**役割分担の原則：** 「注意喚起＝赤」ではなく**アンバー**。色は抑えめにし、青／アンバー／黄マーカーの3役で意味づけする。
...
- カテゴリ色は「区別」のための色。強調（青）・注意（アンバー）の意味とは別レイヤーとして扱い、混同させない。
...
:root {
  ...
  --amber:#b45309; --amber-bg:#fdf0e3; --amber-bd:#e8c9a3;  /* 厳格化・注意（赤の代わり） */
...
- `.Codex/` の設定ファイル（settings.local.json, launch.json）はコミットしない
